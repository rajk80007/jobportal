<div>
    <div class="h-2 text-black border-2 border-black shadow-xl fill-black "></div>
    <h1 class="text-3xl font-bold text-center border-[1px] border-white text-gray-100 p-1"> Top Players of the month</h1>
    <div
        class="grid items-center justify-center grid-cols-1 gap-5 px-8 mx-10 mt-10 md:p-4 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 lg:gap-6">
        <div class="flex flex-col items-center justify-center col-span-1 p-5 bg-white rounded-lg shadow-md">
            <img src="./images/profile.jpg" width="120" height="120" alt="img" class="p-2 rounded-full">
            <h3 class="text-2xl font-bold">John Doe</h3>
            <h4>Frontend Developer</h4>
            <h5>Experience : <span>5 years</span></h5>
            <h6>Rating : <span>5.0</span></h6>
            <h4 class="text-xl font-bold">Skills</h4>
            <p class="w-32 text-sm text-justify text-gray-500 text-wrap">Lorem ipsum dolor sit amet consectetur
                adipisicing
                elit.</p>
        </div>

        {{-- Temporary Cards --}}

        <div class="flex flex-col items-center justify-center col-span-1 p-5 bg-white rounded-lg shadow-md">
            <img src="./images/profile.jpg" width="120" height="120" alt="img" class="p-2 rounded-full">
            <h3 class="text-2xl font-bold">John Doe</h3>
            <h4>Frontend Developer</h4>
            <h5>Experience : <span>5 years</span></h5>
            <h6>Rating : <span>5.0</span></h6>
            <h4 class="text-xl font-bold">Skills</h4>
            <p class="w-32 text-sm text-justify text-gray-500 text-wrap">Lorem ipsum dolor sit amet consectetur
                adipisicing
                elit.</p>
        </div>
        <div class="flex flex-col items-center justify-center col-span-1 p-5 bg-white rounded-lg shadow-md">
            <img src="./images/profile.jpg" width="120" height="120" alt="img" class="p-2 rounded-full">
            <h3 class="text-2xl font-bold">John Doe</h3>
            <h4>Frontend Developer</h4>
            <h5>Experience : <span>5 years</span></h5>
            <h6>Rating : <span>5.0</span></h6>
            <h4 class="text-xl font-bold">Skills</h4>
            <p class="w-32 text-sm text-justify text-gray-500 text-wrap">Lorem ipsum dolor sit amet consectetur
                adipisicing
                elit.</p>
        </div>
        <div class="flex flex-col items-center justify-center col-span-1 p-5 bg-white rounded-lg shadow-md">
            <img src="./images/profile.jpg" width="120" height="120" alt="img" class="p-2 rounded-full">
            <h3 class="text-2xl font-bold">John Doe</h3>
            <h4>Frontend Developer</h4>
            <h5>Experience : <span>5 years</span></h5>
            <h6>Rating : <span>5.0</span></h6>
            <h4 class="text-xl font-bold">Skills</h4>
            <p class="w-32 text-sm text-justify text-gray-500 text-wrap">Lorem ipsum dolor sit amet consectetur
                adipisicing
                elit.</p>
        </div>
        <div class="flex flex-col items-center justify-center col-span-1 p-5 bg-white rounded-lg shadow-md">
            <img src="./images/profile.jpg" width="120" height="120" alt="img" class="p-2 rounded-full">
            <h3 class="text-2xl font-bold">John Doe</h3>
            <h4>Frontend Developer</h4>
            <h5>Experience : <span>5 years</span></h5>
            <h6>Rating : <span>5.0</span></h6>
            <h4 class="text-xl font-bold">Skills</h4>
            <p class="w-32 text-sm text-justify text-gray-500 text-wrap">Lorem ipsum dolor sit amet consectetur
                adipisicing
                elit.</p>
        </div>


    </div>
</div>
